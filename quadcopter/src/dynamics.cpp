...
{
void Dynamics::Init(Eigen::VectorXd x0)
{
    // Initial conditions
...
void Dynamics::GetMotorForces(Eigen::VectorXd u)
{
    ...
    Eigen::VectorXd u_max_slew =
        vehicle_params_.motor_slew_rate * Eigen::VectorXd::Ones(4);
    ...
    Eigen::VectorXd w_motors = vehicle_params_.max_omega * u;

    // Thrust = K_f * w_i^2 for the i-th motor
    Eigen::VectorXd new_x_motors =
        vehicle_params_.K_f * w_motors.array().pow(2);

    // Disturb thrust generated
    for (int i = 0; i < 4; ++i)
    {
        std::normal_distribution<double> motor_dist(env_params_.dist_mean,
...
void Dynamics::UpdateDynamics(Eigen::VectorXd u0)
{
    // States
    double u = x(0);
    double v = x(1);
    ...
    */
    ...
    double arm_cg_len =
        sqrt(pow(vehicle_params_.dx_arm, 2) + pow(vehicle_params_.dy_arm, 2));

    // "Virtual" control
    double Fz = F1 + F2 + F3 + F4;
    double L = (F1 - F2 - F3 + F4) * vehicle_params_.dy_arm;
    double M = (F1 - F2 + F3 - F4) * vehicle_params_.dx_arm;

    // w^2 = F / K_f
    // torque = K_m w^2 = K_b (F / K_f)
    // Note: this assumes CW is positive
    double N = vehicle_params_.K_m * (-F1 - F2 + F3 + F4) / vehicle_params_.K_f;
    ...
    // Velocity update
    x_dot(0) = -env_params_.g * sin_pitch + r * v - q * w;
    x_dot(1) = env_params_.g * sin_roll * cos_pitch - r * u + p * w;
    x_dot(2) = (1 / vehicle_params_.m) * Fz -
               env_params_.g * cos_roll * cos_pitch - q * u + p * v;

    // Angular velocity update
    x_dot(3) = (1 / vehicle_params_.Ixx) *
               (L + (vehicle_params_.Iyy - vehicle_params_.Izz) * q * r);
    x_dot(4) = (1 / vehicle_params_.Iyy) *
               (M + (vehicle_params_.Izz - vehicle_params_.Ixx) * p * r);
    x_dot(5) = (1 / vehicle_params_.Izz) *
               (N + (vehicle_params_.Ixx - vehicle_params_.Iyy) * p * q);
...
void Dynamics::SetVehicleParams(const VehicleParameters &params)
{
    ...
    vehicle_params_.K_f =
        vehicle_params_.max_motor_force /
        (vehicle_params_.max_omega * vehicle_params_.max_omega);
    ...
    vehicle_params_.K_m = vehicle_params_.K_f / vehicle_params_.Kf_divisor_;

    // Save the parameters that are currently set.
    SaveParams();
}

void Dynamics::SetEnvironmentParams(const EnvironmentParameters &params)
{
    env_params_ = params;
...
VehicleParameters Dynamics::GetVehicleParameters() { return vehicle_params_; }

EnvironmentParameters Dynamics::GetEnvironmentParameters()
{
    return env_params_;
}

void Dynamics::SaveParams()
{
    std::string param_string =
        ""
        "// Set the mass\n"
        "m = " +
        std::to_string(vehicle_params_.m) +
        "; // kg\n"
        "\n"
        "// Set gravity\n"
        "g = " +
        std::to_string(env_params_.g) +
        ";\n"
        "\n"
        "// Set the inertia\n"
        "Ixx = " +
        std::to_string(vehicle_params_.Ixx) +
        "; // kg/m^2\n"
        "Iyy = " +
        std::to_string(vehicle_params_.Iyy) +
        "; // kg/m^2\n"
        "Izz = " +
        std::to_string(vehicle_params_.Izz) +
        "; // kg/m^2\n"
        "\n"
        "// Limit the motor's slew rate\n"
        "motor_slew_rate = " +
        std::to_string(vehicle_params_.motor_slew_rate) +
        ...
        "// pull. (Force = 1.40785 * 9.81 = 13.811).\n"
        "max_motor_force = " +
        std::to_string(vehicle_params_.max_motor_force) +
        ";\n"
        "\n"
        "// Max RPM (no-load) is 2750 KV * 14.8V = 40,700 RPM -> 4262 rad/s.\n"
        "// Assuming a load makes for 80% max speed -> 3409.6 rad/s\n"
        "max_omega = " +
        std::to_string(vehicle_params_.max_omega) +
        ";\n"
        "\n"
        ...
        "Kf_divisor_ = " +
        std::to_string(vehicle_params_.Kf_divisor_) +
        ";\n"
        "\n"
        "// Simplify Ixy, Ixz, Iyz = 0 since they are all very small\n"
        "Ixy = " +
        std::to_string(vehicle_params_.Ixy) +
        ";\n"
        "Ixz = " +
        std::to_string(vehicle_params_.Ixz) +
        ";\n"
        "Iyz = " +
        std::to_string(vehicle_params_.Iyz) +
        ";\n"
        "\n"
        "// Arm length\n"
        "dx_arm = " +
        std::to_string(vehicle_params_.dx_arm) +
        ";\n"
        "dy_arm = " +
        std::to_string(vehicle_params_.dy_arm) +
        "; // m\n"
        "\n"
        "// Disturbances\n"
        "dist_mean = " +
        std::to_string(env_params_.dist_mean) +
        ";\n"
        "dist_stddev = " +
        std::to_string(env_params_.dist_stddev) + ";\n";
